...
class _Options:
    """Runtime configuration for kuznets.
    ...
    headers : dict mapping str to str
        ...
        as something other than ``kuznets``.
...
def config_path() -> Path:
    ...
    return Path(base) / "kuznets" / "config.toml"
...
def _file_config() -> dict:
    ...
    if path.is_file():
        try:
            ...
            warnings.warn(f"Ignoring unreadable kuznets config at {path}: {exc}", stacklevel=2)
    _config_cache = data
    return data
...
def get_api_key(source: str, api_key: str | None = None, required: bool = True) -> str | None:
    ...
    if required:
        ...
        raise ValueError(
            ...
            f"kuznets.options.api_keys[{source!r}], the {env} environment variable, "
